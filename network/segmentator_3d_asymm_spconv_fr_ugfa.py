...
class Asymm_3d_spconv(nn.Module):
    ...
    def forward_ow(self, voxel_features, coors, batch_size):
        ...
        # 3. 计算 OSS 分支的 Logits
        # 由于第1步只返回了特征，我们需要手动过一下 OSS 的 Head (PrototypeLinearHead)
        features_oss_norm = self.decoder_ow.logits_arcface(feature_oss.features)
        y_out = feature_oss.replace_feature(features_oss_norm)
        y_out = y_out.dense() # 转为 Dense Tensor 输出

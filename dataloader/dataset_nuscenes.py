...
class cylinder_dataset_nuscenes_panop(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if (intervals == 0).any(): print("Zero interval!")
        grid_ind = (np.floor((np.clip(xyz_pol, min_bound, max_bound) - min_bound) / intervals)).astype(int) # m
...
def collate_fn_BEV(data):
    ...
    label2stack = np.stack([d[1] for d in data]).astype(int) # m
    grid_ind_stack = [d[2] for d in data]
    point_label = [d[3] for d in data]
    xyz = [d[4] for d in data]
    dis_labels = np.stack([d[5] for d in data]).astype(int) # m

...
class cylinder_dataset_panop(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if (intervals == 0).any(): print("Zero interval!")
        grid_ind = (np.floor((np.clip(xyz_pol, min_bound, max_bound) - min_bound) / intervals)).astype(int) # modified
...
def collate_fn_BEV(data):
    ...
    label2stack = np.stack([d[1] for d in data]).astype(int) # modified
